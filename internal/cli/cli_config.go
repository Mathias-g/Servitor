...
// These commands manage the declared config (ADR-0018): the local file that
// declares which connectors (MCP servers, Singer taps and targets) and secrets
// are set up on this box. They edit the config directly; the actual software
// install is delegated to the ecosystem's package managers, which the
...
// configPath returns the config path from the --file flag, or the
// default.
func configPath(fs *flag.FlagSet, args []string) (string, []string, int) {
	path := config.DefaultFile
	fs.StringVar(&path, "file", config.DefaultFile, "path to the config")
...
// cmdSecret manages the declared secrets section of the config
...
func cmdSecretAdd(args []string, stdout, stderr io.Writer) int {
	...
	cfg.AddSecret(positionals[0], &config.Secret{
