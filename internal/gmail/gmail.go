// Package gmail is the Google Workspace (Gmail) helper for the `email_received`
// trigger (SPEC: Triggers). It knows how to talk to Gmail's IMAP server: how to
// connect, authenticate, find new (unseen) messages, and parse each into a
// structured payload. The trigger stays generic; this helper owns the
// provider-specific transport and auth (ADR-0027).
//
