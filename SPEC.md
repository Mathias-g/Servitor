...
- `mcp-http`. Invoke one named tool on one named MCP server over Streamable
  ...
  Wafer. The mechanism is registered (so it validates and appears in
  `capabilities`), but its executor is not yet built: running an `mcp-http`
  node fails with a clear "not yet built" error until the Streamable HTTP
  client and connector lookup are implemented (PLAN Phase 17).
...
and the server is self-hostable. The `mcp-http` executor is not yet built;
this is the planned path once it is.)
