...
servitor enable <name>              # register a workflow's triggers
 servitor disable <name>             # unregister a workflow's triggers without deleting it (this is distinct from disabling a mechanism in the config, SPEC: Disable mechanisms)
...
bundle becomes unreadable once long. An unknown profile name is a Wafer-syntax
error, rejected at validation (dry-run and submit). A node whose requested
profile cannot be satisfied because the host lacks a prerequisite (such as
unprivileged user namespaces or subuid ranges) fails loudly at submit (and at
first run), when the daemon checks the host, never silently degrades to a
...
6. **Workers claim and execute.** A node executor in the parent process claims a job and checks the node's `dedupe_key` against the dedupe table. It spawns a subprocess with a filtered env containing only the secrets the node declared (every node runs as a subprocess; ADR-0008). A node that names an execution profile (SPEC: Execution surface and configuration) is spawned through the launcher with that profile's execution parameters applied (containment, egress, resources, identity); a node whose profile the host cannot satisfy failed at submit (or first run), so it is never claimed. A node whose `type` is a disabled capability is not spawned at all, it is rejected at validation.
7. **Node runs.** Node types dispatch to handlers: HTTP, shell, transform, Singer tap, Singer target, helpers, and the flow nodes `switch` (route to one branch) and `foreach` (fan a body out over a list). A node whose `type` is a declared flavor resolves to its base mechanism's harness with the flavor's pinned parameters applied (SPEC: Mechanism flavors). A node writes its result as structured JSON to stdout and exits. A `switch` and `foreach` resolve their decision in a subprocess and then route: the worker fans out the chosen branch / body iterations through the dependency counters (ADR-0022, ADR-0024).
...
What works today (the current-state snapshot) is tracked in [STATUS.md](STATUS.md).
This spec is the behavior contract (the target), not a build-status ledger, so
it does not annotate features as built or not. See [PLAN.md](PLAN.md) for the
build order and per-phase "done" tracking.
