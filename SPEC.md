...
6. **Workers claim and execute.** A node executor in the parent process claims a job and checks the node's `dedupe_key` against the dedupe table. It spawns a subprocess with a filtered env containing only the secrets the node declared (every node runs as a subprocess; ADR-0008). A node that names an execution profile (SPEC: Execution surface and configuration) is spawned through the launcher with that profile's execution parameters applied (containment, egress, resources, identity); a node whose profile the host cannot satisfy fails loudly at validation or submit, so it is never claimed. A node whose `type` is a disabled capability is not spawned at all, it is rejected at validation.
