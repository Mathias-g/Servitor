...
- **Run the checks with `make check`** (or `make test` for tests alone): `go test ./...`, `go vet ./...`, `golangci-lint run`, and `gofmt -l`. These are the same checks CI runs. Two setup notes for a fresh box: `make check` invokes `golangci-lint` by name, so it must be on `PATH` (run it with `PATH="$HOME/go/bin:$PATH"` if `go install` put it there); and `make test`/`make check` now **fail loudly** if `HONKER_EXTENSION_PATH` is not set, because the Honker-backed tests (worker, honker, daemon, runner, trigger) would otherwise silently skip and report a weak pass. Set it to a loadable `libhonker_ext.so`, which CI supplies (ADR-0011).
